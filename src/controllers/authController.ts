...
export const logout = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Store token in Redis with an expiration time (matching the token lifespan)
    const expirationTime = 3600; // 1 hour (should match access token expiry)
    ...
    res
      ...
      .status(204)
      .json({ message: SUCCESS_MESSAGES.LEAD_DELETED });
  } catch (error) {
    res.status(500).json({ error: ERROR_MESSAGES.INTERNAL_SERVER_ERROR });
  }
};
